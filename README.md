...
当前公开测试版：**0.2.1（Build 13）**

[前往 GitHub Releases 下载 LocalShot](https://github.com/tpxcer/LocalShot-Downloads/releases/tag/v0.2.1-build13)

下载文件：`LocalShot-0.2.1-build13-offline-license-arm64.dmg`

> 当前版本仅支持 Apple 芯片 Mac，最低需要 macOS 13。安装包尚未经过 Apple 公证，首次打开时需要按照[安装说明](INSTALL.md)完成系统确认。

> Build 4 及更早版本没有更新检查功能，需要手动安装 Build 13；Build 5–12 用户可直接在软件内收到 Build 13 更新提示。
...
在 macOS 15 或更高版本中选择需要录制的窗口或屏幕区域。单击自动识别的窗口会锁定该窗口，移动或缩放窗口时录制内容、红框和控制条会跟随；拖动自由框选则固定在屏幕位置。悬浮控制条提供录制计时、系统声音、麦克风、取消和开始/停止按钮；系统声音默认开启，麦克风默认关闭。完成后视频以 H.264 MP4 保存在本机独立录制目录。
...
- 区域录屏：自动识别并锁定窗口，或自由框选固定屏幕区域；持续显示红框和计时，可选择系统声音与麦克风。
...
3. 按需选择系统声音和麦克风，然后点击“开始录制”。
4. 点击红色“停止录制”完成保存，LocalShot 会在 Finder 中定位 MP4。

录制文件保存在 `Application Support/LocalShot/Recordings`，不进入截图历史，也不会被“清空截图记录”删除。区域录屏需要 **macOS 15 或更高版本**。
...
1. 从 [Build 13 Release](https://github.com/tpxcer/LocalShot-Downloads/releases/tag/v0.2.1-build13) 下载 DMG。
...
```bash
shasum -a 256 -c SHA256SUMS-v0.2.1-build13-public.txt
```
...
- 只提供区域 MP4 录制，不提供 GIF、独立窗口或全屏视频录制，也不提供云端分享、团队协作或跨设备同步。
