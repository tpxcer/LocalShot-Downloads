...
当前公开测试版：**0.3.5（Build 19）**

[前往 GitHub Releases 下载 LocalShot](https://github.com/tpxcer/LocalShot-Downloads/releases/tag/v0.3.5-build19)

下载文件：`LocalShot-0.3.5-build19-offline-license-universal2-arm64-x86_64.dmg`

> 当前版本同时支持 Apple 芯片和 Intel Mac，最低需要 macOS 13。安装包尚未经过 Apple 公证，首次打开时需要按照[安装说明](INSTALL.md)完成系统确认。

> Build 4 及更早版本没有更新检查功能，需要手动安装 Build 19；Build 5–18 用户可直接在软件内收到 Build 19 更新提示。
...
在 macOS 15 或更高版本中选择需要录制的窗口或屏幕区域。单击自动识别的窗口会锁定该窗口，移动或缩放窗口时录制内容、红框和控制条会跟随；拖动自由框选则固定在屏幕位置。开始前显示 3、2、1 倒计时；录制中可暂停/继续，也可使用全局 `⌘⇧R` 停止。画质支持标准、高清和原画，帧率支持 30/60 FPS，并显示预计文件增长和磁盘可用空间。完成后视频以 H.264 MP4 与截图保存在同一个本地目录。
...
- 状态栏“功能”分组提供区域、窗口、全屏截图、识别文字和区域录屏；长截图继续从截图选区工具栏进入。
...
1. 从菜单“功能”中选择“区域录屏”，或在截图选区工具栏点击圆形录制图标。
...
控制条会按当前画质与帧率显示每分钟文件大小估算范围和磁盘可用空间。空间达到安全线时，LocalShot 会自动停止并保存已有内容。截图和新录屏默认保存在 `Application Support/LocalShot/Captures`；如果设置了自定义目录，两者会一起跟随该目录。录屏会显示在“最近保存”中，但不会进入截图缩略图历史。旧版本 `Application Support/LocalShot/Recordings` 中的文件不会自动移动或删除。区域录屏需要 **macOS 15 或更高版本**。

“清空记录”不勾选删除文件时，只隐藏截图和录屏记录，原文件继续保留；勾选“同时删除截图和录屏文件”后，才会删除当前目录内由 LocalShot 生成的截图、标注项目和最终 MP4。个人视频、临时录制分段和旧版独立录屏目录不在删除范围内。
...
- “最近保存”按时间同时显示截图和录屏；点击截图打开快速访问，点击录屏在 Finder 中定位。
- 支持 PNG/JPEG、自定义文件名前缀、截图与录屏共用保存目录、是否包含光标和窗口阴影。
...
Build 19 使用同一个 Universal 2 安装包覆盖两种处理器架构，不需要区分下载版本。

## 安装与首次使用

1. 从 [Build 19 Release](https://github.com/tpxcer/LocalShot-Downloads/releases/tag/v0.3.5-build19) 下载 DMG。
...
```bash
shasum -a 256 -c SHA256SUMS-v0.3.5-build19-public.txt
```
