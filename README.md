...
当前公开测试版：**0.3.3（Build 17）**

[前往 GitHub Releases 下载 LocalShot](https://github.com/tpxcer/LocalShot-Downloads/releases/tag/v0.3.3-build17)

下载文件：`LocalShot-0.3.3-build17-offline-license-universal2-arm64-x86_64.dmg`

> 当前版本同时支持 Apple 芯片和 Intel Mac，最低需要 macOS 13。安装包尚未经过 Apple 公证，首次打开时需要按照[安装说明](INSTALL.md)完成系统确认。

> Build 4 及更早版本没有更新检查功能，需要手动安装 Build 17；Build 5–16 用户可直接在软件内收到 Build 17 更新提示。
...
控制条会按当前画质与帧率显示每分钟文件大小估算范围和磁盘可用空间。空间达到安全线时，LocalShot 会自动停止并保存已有内容。截图和新录屏默认保存在 `Application Support/LocalShot/Captures`；如果设置了自定义目录，两者会一起跟随该目录。录屏不进入截图历史，也不会被“清空记录”删除。旧版本 `Application Support/LocalShot/Recordings` 中的文件不会自动移动或删除。区域录屏需要 **macOS 15 或更高版本**。
...
Build 17 使用同一个 Universal 2 安装包覆盖两种处理器架构，不需要区分下载版本。

## 安装与首次使用

1. 从 [Build 17 Release](https://github.com/tpxcer/LocalShot-Downloads/releases/tag/v0.3.3-build17) 下载 DMG。
...
```bash
shasum -a 256 -c SHA256SUMS-v0.3.3-build17-public.txt
```
